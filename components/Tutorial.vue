  ...
  <div class="lg:min-w-lg w-[60vw]">
    ...
    <div v-if="currentStep.showInput" class="my-2">
      <label class="mt-4 flex flex-col">
        ...
        <input v-on:keyup.enter="submitInput" ref="inputAnswer" value=""
          class="disabled:bg-green-300 mt-1 p-2 border rounded-md" type="text" :placeholder="currentStep.inputValue">
      </label>
      <div class="flex justify-between"><button
          class="mt-2 bg-teal-500 hover:bg-teal-600 text-white py-2 px-4 rounded-md"
          @click="submitInput">Submit</button>
        <p class="font-maple-mono italic text-lg" v-if="wasRight">Good Job!</p>
      </div>
    </div>
    ...
    <div class="flex justify-between">
      <button class="disabled:bg-gray-600 bg-teal-500 hover:bg-teal-600 text-white py-2 px-4 rounded-md"
        @click="previousStep" :disabled="currentStepIndex === 0">
        Previous
      </button>
      <button class="disabled:bg-gray-600 bg-teal-500 hover:bg-teal-600 text-white py-2 px-4 rounded-md" @click="run"
...
export default {
  ...
  methods: {
    ...
    update() {
      ...
      this.wasRight = false;
      this.wasWrong = false;
      this.$refs.inputAnswer.disabled = false;
    },
    run() {
